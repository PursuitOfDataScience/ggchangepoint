...
jump, `pelt` returns 1 changepoint at sigma = 1 but 39 at sigma = 3 and 141
at sigma = 10 (means over 20 draws; a single draw is not stable here).
...
estimates a variance per segment. The other engines estimate or cancel
the noise scale themselves and are unaffected, with three exceptions:
`geomcp` (it runs PELT), and `decafs` and `bocpd` on very small units. See
