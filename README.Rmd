...
```{r, fig.alt = "Series with a date axis and a changepoint rule, detected with PELT"}
...
```{r, eval = requireNamespace("nsp", quietly = TRUE), fig.alt = "Series with a shaded vertical band marking an NSP significance region"}
...
```{r, fig.alt = "Small multiples showing how the segmentation coarsens as the number of changepoints falls, with the chosen model marked"}
...
```{r, eval = requireNamespace("mosum", quietly = TRUE), fig.alt = "Two-panel display: the series above, the MOSUM statistic against its threshold below"}
res_mosum <- cpt_detect(c(rnorm(200), rnorm(200, 3)), method = "mosum")
autoplot(res_mosum, type = "statistic")
```

```{r, eval = requireNamespace("mosum", quietly = TRUE), fig.alt = "Heatmap of the MOSUM statistic by location and bandwidth, with accepted changepoints marked"}
...
```{r, fig.alt = "Series with each observation sized and coloured by how much perturbing it disturbs the segmentation"}
...
```{r, fig.alt = "One facet per penalty setting, each showing the changepoints that setting finds"}
...
```{r, fig.alt = "Series with shaded label regions behind it, coloured by whether the label asserts a change"}
...
```{r, fig.alt = "Dot matrix of method against location, with shaded bands marking the consensus changepoints"}
...
```{r, fig.alt = "Heatmap of the covering metric for each method on each benchmark dataset"}
...
```{r, fig.alt = "Power curve: detection probability against change size, with a Monte Carlo interval"}
