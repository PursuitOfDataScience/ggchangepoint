...
#>   Alarms: 0
#>   Restart after alarm: TRUE (re-learning 20 observations)
```
...
observation 121 of the monitored stream, and the monitor sees it only
...
matches each alarm to the change it plausibly detected, and counts the
rest as false alarms.
...
is the wrong tool here. It asks whether the *location* was recovered,
which a sequential procedure never claims: an alarm is deliberately
late, so scoring it as a location estimate marks a correct detection
wrong by exactly the delay.
...
![Monitored series with dashed vertical lines at each
alarm](monitoring_files/figure-html/timeline-1.png)
...
![Bar chart of the detection delay at the true
changepoint](monitoring_files/figure-html/delay-plot-1.png)

## 3. Replaying a finished series

[`cpt_replay()`](https://pursuitofdatascience.github.io/ggchangepoint/reference/cpt_replay.md)
does the update loop for you: it takes the leading observations as the
baseline, streams the rest, and returns the same monitor object. This is
how to study an online method’s alarm timeline retrospectively without
pretending its alarms are changepoints.
...
long to notice a 0.75-sigma one, while the mixture is close to the best
...
and a threshold calibrated to a target in-control average run length.
...
`NA` in its alarm log rather than an invented number, and the
`statistic` column is not informative for it either. The alarm *time* is
the whole signal from `cpm`; the statistic trace is what `edetector` and
`ocd` provide and `cpm` does not.
...
exposes both because the two engines are calibrated in different
currencies, and each ignores the other’s. **`edetector` honours
`alpha`** (its threshold is $`1/\alpha`$); **`cpm` honours `arl0`**.
Setting `arl0` on an e-detector, or `alpha` on `cpm`, changes nothing.
...
is lowered here. Pass `thresh` directly when you already have
thresholds.
...
`edetector` **11.5**. The spread matters as much as the mean, and is the
reason to distrust a single stream: cpm’s counts ranged from 0 to 7
across the 20 (sd 2.0) and the e-detector’s from 1 to 19 (sd 5.2), so
agreement to within one alarm is not something one run can establish.
...
The baseline must be **clean**: `edetector` and `ocd` take their
reference mean and scale from it, so a change inside the baseline is
inherited as the in-control state and the real change afterwards may be
...
sequential monitoring since Page (1954); the tooling here just makes the
...
Shin, Jaehyeok, Aaditya Ramdas, and Alessandro Rinaldo. 2023.
“E-Detectors: A Nonparametric Framework for Sequential Change
Detection.” *The New England Journal of Statistics in Data Science* 1
(2): 229–60. <https://doi.org/10.51387/23-NEJSDS49>.
