...
renders a publication table. `geom_cpt_event()` draws the events;
`scale_colour_cpt()`, `scale_fill_cpt()`, `scale_linetype_cpt()`,
`scale_fill_cpt_label()` and `scale_colour_cpt_label()` are the
colour-vision-safe scales; `theme_ggcpt()` is unchanged.
...
`cpt_min_detectable()` inverts the curve, and `cpt_scenarios()` builds a
grid of settings as data.
